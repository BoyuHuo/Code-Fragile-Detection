...
public class FlaskSegmentService implements SegmentService {
  ...
  public File run(String pictureName) {
    ...
    String outputPath =
        Paths.get(WORKSPACEPATH, SEGPICBASEPATH, basePictureName + "_seg.png").toString();
    String groundTruthPath =
        Paths.get(WORKSPACEPATH, GROUNDTRUTHBASEPATH, basePictureName + "_segmentation.png")
            .toString();

    String body =
        String.format(
            "{\"model_path\":\"%s\", \"test_pic_path\":\"%s\", \"output_image_path\":\"%s\", \"ground_truth_path\":\"%s\"}",
            modelPath, picturePath, outputPath, groundTruthPath);
