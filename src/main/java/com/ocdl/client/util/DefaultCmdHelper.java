...
public class DefaultCmdHelper implements CommandHelper {
  ...
  /**
   * The class is to read inputStream buffer from process's std / stderr output in a independent
   * thread. Problem solved: process can not continue to output, when inputStream buffer is full ->
   * process wait JVM to read, while JVM wait process to terminate, causing deadlock, if process
  ...
  private class ReadStreamBuffer extends Thread {
    ...
    public void run() {
      BufferedReader bufferedReader =
          new BufferedReader(new InputStreamReader(this.readInputStream));
      try {
